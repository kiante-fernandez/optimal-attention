...
ggsave(here("figures", "density_total_fixation_time.png"), dpi = 300)

# Histogram of number of fixations in a trial
mung_df %>%
  group_by(k, trial_idx) %>%
  mutate(fixation_num = cumsum(c(0, as.numeric(diff(fixations)) != 0)) + 1) %>%
  slice_max(fixation_num) %>%
  slice_tail(n = 1) %>%
  ggplot(aes(fixation_num, y = ..density.., color = factor(k), group = factor(k), fill = factor(k))) +
  geom_histogram(color = "black", binwidth = .99) +
  labs(
...
mean_value <- full_df %>%
  ...
  group_by(k, trial_idx) %>%
  mutate(mean_value = mean(item_value)) %>%
  select(k, trial_idx, mean_value) %>%
  distinct()
...
mung_df %>%
  ...
  summarise(rt = sum(fixation_duration)) %>%
  left_join(mean_value, by = c("trial_idx", "k")) %>%
  mutate(mean_value = round(mean_value)) %>% # round for plotting?
  ggplot(aes(mean_value, rt, color = factor(k), group = factor(k))) +
  ...
  scale_color_brewer(palette = "Set1") +
  facet_wrap(~k, labeller = "label_both", scales = "fixed")
ggsave(here("figures", "fixation_time_mean_value.png"), dpi = 300)

# Total fixation time as a function of the relative rating of the highest rated item
# NOTE: this can take a long time, mutate that uses subset is computationally intensive
...
full_df %>%
  ...
  select(k, trial_idx, relative_rating) %>%
  distinct() %>%
  ...
  scale_color_brewer(palette = "Set1") +
  facet_wrap(~k, labeller = "label_both", scales = "fixed")
ggsave(here("figures", "fixation_time_relative_value.png"), dpi = 300)
...
full_df %>%
  ...
  facet_wrap(~k, labeller = "label_both", scales = "fixed")
ggsave(here("figures", "first_fixation_first_value.png"), dpi = 300)

# Duration of fixation by fixation number
full_df %>%
  group_by(k) %>%
  mutate(
    trial_idx = seq_len(n()),
  ...
  mutate(timestep_idx = seq_len(n())) %>%
  mutate(
    fixation_num = cumsum(c(0, as.numeric(diff(fixations)) != 0)) + 1, # add one here so now fixation indicator starts at 1
    final_fixation = if_else(fixation_num == max(fixation_num), 1, 0)
  ) %>%
  group_by(k, trial_idx, fixation_num) %>%
  add_tally() %>%
  mutate(fixation_duration = n * 100, .keep = "unused") -> duration_df

duration_df <- duration_df %>%
  select("trial_idx", "k", "fixation_num", "final_fixation", "fixation_duration") %>%
  distinct() %>%
  mutate(
    plot_indicator = ifelse(fixation_num %in% c(1, 2, 3, 4, 5), fixation_num, 6),
    plot_indicator = if_else(final_fixation == 0, plot_indicator, 7)
  )
# for plotting
duration_df$plot_indicator <- factor(duration_df$plot_indicator, labels = c("1", "2", "3", "4", "5", "<5", "final"))
ggplot(duration_df, aes(plot_indicator, fixation_duration, color = factor(k), group = factor(k))) +
  stat_summary(fun.data = mean_se, geom = "errorbar") +
  stat_summary(fun.data = mean_se, geom = "point") +
  stat_summary(fun = mean, geom = "line", size = .7) +
  labs(
    ...
    title = "Duration of fixation by fixation number"
  ) +
  theme_classic() +
  theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  facet_wrap(~k, labeller = "label_both", scales = "fixed")
ggsave(here("figures", "duration_fixation_number.png"), dpi = 300)

# here is a plot of the number of observations for each fixation number
# we would expect that the proportion should get lower at higher fixation numbers
ggplot(duration_df, aes(plot_indicator, group = factor(k), fill = factor(k))) +
